...
#
class Chemical:
    def __init__(self, name, components, produces):
    ...
    def obtain(self, required_amount, chemical_store):
        ...
        # if we're not negative then we are done 
        while 0 > self.quantity_at_hand:
            # ok, we need to produce 
            self.produce(chemical_store)

    def produce(self, chemical_store):
        """
        Produce one batch of this chemical
        """
        # obtain enough of each chemical 
        for required_quantity, required_chemical in self.components:
            chemical_store.obtain(required_chemical, required_quantity)
        # and we have produced
        self.quantity_at_hand += self.produce_quantity
...
class ChemicalStore:
    ...
    def obtain(self, chemical, amount):
        ...
        """
        self.chemicals[chemical].obtain(amount, self)
