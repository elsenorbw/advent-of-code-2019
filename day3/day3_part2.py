...
def instructions_to_locations(instructions):
    """
    Takes a set of instructions and adds all locations to a results list.
    ...
    current_y = 0
    distance = 0
    individual_instructions = instructions.split(',')
    for this_instruction in individual_instructions:
        this_instruction = this_instruction.strip()
        if('' != this_instruction):
            #
            #  get the direction and the magnitude then loop through creating the list 
            #
            direction = this_instruction[0]
            magnitude = int(this_instruction[1:])
            #print(f"Moving {direction} for {magnitude} from {current_x},{current_y}")
            ...
            if('R' == direction):
                ...
                raise ValueError(f"The supplied direction {direction} is not one of the valid directions.")
            # and now loop though magnitude steps adding them to the result 
...
def find_closest_intersection(instructions_a, instructions_b, origin = (0, 0)):
    """
    Return the length and nearest X,Y for closest intersection to origin 
    """
